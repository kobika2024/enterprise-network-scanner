...
## הורדה ושימוש מהיר (EXE מוכן)

1. עברו ל-[Releases](../../releases) בדף הזה
2. הורידו את `NetworkScanner-vX.X.zip`
3. חלצו את הקובץ לתיקייה כלשהי (שמרו את כל הקבצים ביחד)
4. לחצו לחיצה כפולה על `NetworkScanner.exe`
5. אשרו את חלון ה-UAC (בקשת הרשאות Administrator)
